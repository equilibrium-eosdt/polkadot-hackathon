...
export async function loop(
  ...
  for (;;) {
    logger.info(`*** Test case is starting for N=${params.n}, M=${params.m} ***`);
...
export async function suite(
  ...
  if (bads == 0) {
    logger.info(`*** Test case finished successefuly for N=${params.n}, M=${params.m} ***`);
  } else if (goods > bads) {
    logger.warn(`*** Test case finished sub-successefuly (${JSON.stringify({goods, bads})}) for N=${params.n}, M=${params.m} ***`);
  } else if (goods > 0) {
    logger.warn(`*** Test case finished unsuccessefuly (${JSON.stringify({goods, bads})}) for N=${params.n}, M=${params.m} ***`);
  } else {
    throw new Error(`*** Test case failed for N=${params.n}, M=${params.m} ***`);
  }
